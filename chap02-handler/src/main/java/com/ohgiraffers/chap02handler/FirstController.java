...
public class FirstController {
     ...
     * 뷰를 해석하려고 하는데 예를 들어 "/example"이라는  url로 요청이 들어오고 그 해당하는 컨드롤러가
     ...
     *   핸들러 메소드 매개변수로 HttpSerlvetRequestm HttpServletResponse도 사용 가능하다.
     *   상위 타입인 ServletRequestm ServletReponse도 사용 가능하다.
     *   WebRequest는 HttpServeltRequest의 요청 정보를 거의 대부분 그대로 가지고 있는 API로 Serlvet에 종속적이지 않다.
    ...
    public ModelAndView modifyMenu(ModelAndView mv,
                                   @RequestParam(name = "modifyName",required = false) String modifyMenu,
        ...
        //  String modifyMenu = webRequest.getParameter("modifyName").equals(null) ? option? defaultValue:"" :webRequest.getParameter("modifyName");

        String message = modifyMenu + "메뉴의 가격을 " + menuPrice + "원으로 가격을 변경하였습니다.";
        System.out.println(message);
